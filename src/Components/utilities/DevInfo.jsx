  ...
  if (state.currentMenu === "HOME") {
      ...
      if (state.devList.length > 0) {
        switch (state.devList[i].info.Model) {
          case "MIDI GPIO":
            deviceList.push(
              <MenuItem
                key={"midiGpioMenuItem" + i}
                icon={gpioIcn}
                onClick={() => doThis("GPIO", "SETTINGS", serNum)}
              >
                MIDI GPIO
              </MenuItem>
            );
    ...
    side.push(
      <Menu key={"sidebar" + i} iconShape="circle">
        <MenuItem icon={backIcn} onClick={() => doThis("HOME")}>
          Back
        </MenuItem>
        <MenuItem
          icon={infoIcn}
          onClick={() => doThis("GPIO", "SETTINGS", state.currentDevice)}
        >
          Settings
        </MenuItem>
        <MenuItem
          icon={matrix}
          onClick={() => doThis("GPIO", "ROUTE", state.currentDevice)}
        >
          Routing Matrix
        </MenuItem>
