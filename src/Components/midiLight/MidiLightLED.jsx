...
export default function MidiLightLED(props) {
  ...
  const aSlider = sliderType => {
    ...
    return (
      <table
        key={sliderType + "SliderCh" + props.channel}
        style={{ width: "100%" }}
      >
        <tbody>
          <tr>
            <td style={label}>Val </td>
            <td
              style={{
                width: "100%",
                padding: "0px 12px",
    ...
    if (showColorPicker) {
      ...
      switch (channel.ledData.type.label) {
        case "Solid":
          return (
            <Fragment>
              {stylePicker()} {horizontalLine()}
              {slot("Rise:", aSlider("Rise"))} {horizontalLine()}
        ...
        case "Flash":
          return (
            <Fragment>
              {stylePicker()} {horizontalLine()}
              {slot("Drtn:", aSlider("Drtn"))} {horizontalLine()}
              {slot("Rise:", aSlider("Rise"))} {horizontalLine()}
              {slot("Fall:", aSlider("Fall"))}
            </Fragment>
          );
        ...
        case "Blink":
          return (
            <Fragment>
              {stylePicker()} {horizontalLine()}
              {slot("Freq:", aSlider("Freq"))} {horizontalLine()}
              {slot("Len:", aSlider("Len"))} {horizontalLine()}
              {slot("Fall:", aSlider("Fall"))}
            </Fragment>
          );
